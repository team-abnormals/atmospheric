	...
	public void applyEffectTick(LivingEntity entity, int amplifier) {
		...
		if (!entity.level.isClientSide && entity.getHealth() > 0) {
			...
			if (entity.level.getGameTime() % chance == 0) {
				entity.getLevel().playSound(null, entity.getX(), entity.getY(), entity.getZ(), SoundEvents.LLAMA_SPIT, SoundSource.NEUTRAL, 0.5F, 0.4F + (random.nextFloat() - random.nextFloat()) * 0.2F);
