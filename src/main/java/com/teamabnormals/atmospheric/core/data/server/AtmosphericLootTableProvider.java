...
public class AtmosphericLootTableProvider extends LootTableProvider {
	...
	private static class AtmosphericBlockLoot extends BlockLoot {
		...
		private static final float[] CURRANT_LEAVES_STICK_CHANCES = new float[]{0.04F, 0.044444446F, 0.05F, 0.066666670F, 0.2F};
		...
		public void addTables() {
			...
			this.add(ALOE_VERA.get(), (block) -> applyExplosionDecay(block, LootTable.lootTable().withPool(LootPool.lootPool().when(LootItemBlockStatePropertyCondition.hasBlockStateProperties(ALOE_VERA.get()).setProperties(StatePropertiesPredicate.Builder.properties().hasProperty(AloeVeraBlock.AGE, 5))).add(LootItem.lootTableItem(AtmosphericItems.ALOE_LEAVES.get())).apply(SetItemCountFunction.setCount(UniformGenerator.between(1.0F, 2.0F))).apply(ApplyBonusCount.addUniformBonusCount(Enchantments.BLOCK_FORTUNE))).withPool(LootPool.lootPool().when(LootItemBlockStatePropertyCondition.hasBlockStateProperties(ALOE_VERA.get()).setProperties(StatePropertiesPredicate.Builder.properties().hasProperty(AloeVeraBlock.AGE, 5))).add(LootItem.lootTableItem(AtmosphericItems.ALOE_KERNELS.get())).apply(ApplyBonusCount.addBonusBinomialDistributionCount(Enchantments.BLOCK_FORTUNE, 0.5714286F, 3)))));
			...
			this.add(TALL_YUCCA_FLOWER.get(), block -> createDoublePlantShearsDrop(YUCCA_FLOWER.get()));
		...
		protected static Builder createTallAloeVeraDrops(Block block) {
			return applyExplosionDecay(block, LootTable.lootTable()
					...
					.withPool(LootPool.lootPool().setRolls(ConstantValue.exactly(1.0F)).add(LootItem.lootTableItem(AtmosphericItems.YELLOW_BLOSSOMS.get()).apply(List.of(6, 7, 8), (val) -> SetItemCountFunction.setCount(ConstantValue.exactly((float) val)).when(LootItemBlockStatePropertyCondition.hasBlockStateProperties(block).setProperties(StatePropertiesPredicate.Builder.properties().hasProperty(DoublePlantBlock.HALF, DoubleBlockHalf.LOWER).hasProperty(AloeVeraTallBlock.AGE, val))))))
					.withPool(LootPool.lootPool().when(LootItemBlockStatePropertyCondition.hasBlockStateProperties(ALOE_VERA.get()).setProperties(StatePropertiesPredicate.Builder.properties().hasProperty(AloeVeraBlock.AGE, 8).hasProperty(DoublePlantBlock.HALF, DoubleBlockHalf.LOWER))).add(LootItem.lootTableItem(AtmosphericItems.ALOE_KERNELS.get())).apply(ApplyBonusCount.addBonusBinomialDistributionCount(Enchantments.BLOCK_FORTUNE, 0.5714286F, 3)))
	...
	private static class AtmosphericChestLoot extends ChestLoot {
		...
		public void accept(BiConsumer<ResourceLocation, Builder> consumer) {
			consumer.accept(new ResourceLocation(Atmospheric.MOD_ID, "chests/arid_shrine"), LootTable.lootTable()
