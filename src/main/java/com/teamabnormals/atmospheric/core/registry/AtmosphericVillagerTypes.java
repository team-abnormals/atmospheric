...
public class AtmosphericVillagerTypes {
	public static final DeferredRegister<VillagerType> VILLAGER_TYPES = DeferredRegister.create(Registries.VILLAGER_TYPE, Atmospheric.MOD_ID);

	public static final DeferredHolder<VillagerType, ?> SCRUBLAND = register("scrubland");
			...
			for (ItemListing listing : trades.get(TradeUtil.MASTER)) {
				if (listing instanceof VillagerTrades.EmeraldsForVillagerTypeItem trade) {
					...
					if (!trade.trades.containsKey(SCRUBLAND.get())) {
						newTrades.put(SCRUBLAND.get(), AtmosphericItems.LAUREL_BOAT.get());
					}

					if (newTrades.get(VillagerType.DESERT) == Items.JUNGLE_BOAT && AtmosphericConfig.COMMON.yuccaDesertVillages.get()) {
						newTrades.replace(VillagerType.DESERT, AtmosphericItems.YUCCA_BOAT.get());
					}
