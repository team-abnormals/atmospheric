...
public class ColorUtils
{
	public static void registerBlockColors() {
        ...
        blockColors.register((x, world, pos, u) -> world != null && pos != null ? BiomeColors.getFoliageColor(world, pos) : FoliageColors.get(0.5D, 1.0D), AtmosphericBlocks.DUNE_LEAVES.get());
        ItemColors itemColors = Minecraft.getInstance().getItemColors();
        itemColors.register((color, items) -> FoliageColors.get(0.5D, 1.0D), AtmosphericBlocks.ROSEWOOD_LEAVES.get());
        itemColors.register((color, items) -> FoliageColors.get(0.5D, 1.0D), AtmosphericBlocks.DUNE_LEAVES.get());
        ...
        if (ModList.get().isLoaded("quark")) {
            ...
            blockColors.register((x, world, pos, u) -> world != null && pos != null ? BiomeColors.getFoliageColor(world, pos) : FoliageColors.get(0.5D, 1.0D), AtmosphericBlocks.DUNE_LEAF_CARPET.get());
        	itemColors.register((color, items) -> FoliageColors.get(0.5D, 1.0D), AtmosphericBlocks.ROSEWOOD_LEAF_CARPET.get());
        	itemColors.register((color, items) -> FoliageColors.get(0.5D, 1.0D), AtmosphericBlocks.DUNE_LEAF_CARPET.get());
        }
    }
}
