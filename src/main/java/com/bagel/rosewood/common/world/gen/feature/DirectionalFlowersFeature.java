...
public class DirectionalFlowersFeature extends Feature<NoFeatureConfig> {
	   ...
	   public boolean place(IWorld worldIn, ChunkGenerator<? extends GenerationSettings> generator, Random rand, BlockPos pos, NoFeatureConfig config) {
	      BlockState blockstate = RosewoodBlocks.MONKEY_BRUSH.get().getDefaultState();
	      int i = 0;
	      ...
	      for(int j = 0; j < 64; ++j) {
	         ...
	         if (worldIn.isAirBlock(blockpos) && blockpos.getY() < 255 && blockstate.isValidPosition(worldIn, blockpos)) {
	        	...
	        	BlockState blockstate2 = RosewoodBlocks.MONKEY_BRUSH.get().getDefaultState().with(MonkeyBrushBlock.FACING, randomD);
