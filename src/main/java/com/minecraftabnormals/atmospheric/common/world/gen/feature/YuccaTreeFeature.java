...
public class YuccaTreeFeature extends Feature<YuccaTreeFeatureConfig> {
	...
	public boolean generate(ISeedReader worldIn, ChunkGenerator generator, Random rand, BlockPos position, YuccaTreeFeatureConfig config) {
		if (config.baby) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= worldIn.getHeight()) {
				...
				if (!flag) {
					return false;
				} else if (TreeUtil.isInTag(worldIn, position.down(), AtmosphericTags.YUCCA_PLANTABLE_ON) && position.getY() < worldIn.getHeight()) {
					if (!TreeUtil.isInTag(worldIn, position.down(), BlockTags.SAND))
					...
					for (int k1 = 0; k1 < height; ++k1) {
						...
						if (TreeUtil.isAirOrLeaves(worldIn, blockpos)) {
							TreeUtil.setForcedState(worldIn, blockpos, config.trunkProvider.getBlockState(rand, blockpos));
						}
					}
					...
					if (config.patch) {
						for (int j = 0; j < 64; ++j) {
							...
							if (TreeUtil.isAir(worldIn, blockpos) && blockpos.getY() < 255 && config.flowerProvider.getBlockState(rand, blockpos).isValidPosition((IWorldReader) worldIn, blockpos)) {
				...
				if (!flag) {
					...
					if (config.petrified && rand.nextInt(12) == 0) {
						for (int j = 0; j < 12; ++j) {
							...
							if (TreeUtil.isAir(worldIn, blockpos) && blockpos.getY() < 255 && TreeUtil.isInTag(worldIn, blockpos.down(), BlockTags.SAND)) {
								TreeUtil.setForcedState(worldIn, blockpos, config.bundleProvider.getBlockState(rand, blockpos));
							}
						}
					}
	...
	private void createHorizontalLog(int branchLength, IWorldGenerationReader worldIn, BlockPos pos, Direction direction, boolean bundle, Random rand, YuccaTreeFeatureConfig config) {
		...
		for (int k3 = 0; k3 < branchLength; ++k3) {
			...
			if (TreeUtil.isAirOrLeaves(worldIn, blockpos1)) {
				if (!TreeUtil.isAir(worldIn, blockpos1.down()))
					bundle = false;
	...
	private void placeLogAt(IWorldWriter worldIn, BlockPos pos, Direction direction, boolean bundle, Random rand, YuccaTreeFeatureConfig config) {
		...
		if (bundle && !config.petrified) {
			TreeUtil.setForcedState(worldIn, pos.down(), config.branchProvider.getBlockState(rand, pos.down()));
		}
	}
	...
	private void placeFlowerAt(IWorldGenerationReader world, BlockPos pos, Random rand, YuccaTreeFeatureConfig config) {
		if (TreeUtil.isAir(world, pos)) {
			if (!TreeUtil.isAir(world, pos.up())) {
				TreeUtil.setForcedState(world, pos, config.flowerProvider.getBlockState(rand, pos));
			} else if (rand.nextInt(4) == 0) {
				TreeUtil.setForcedState(world, pos, config.tallFlowerBottomProvider.getBlockState(rand, pos));
				TreeUtil.setForcedState(world, pos.up(), config.tallFlowerTopProvider.getBlockState(rand, pos.up()));
			} else {
				TreeUtil.setForcedState(world, pos, config.flowerProvider.getBlockState(rand, pos));
			}
		}
